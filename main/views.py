...
def email_change(request):
    if request.method == "GET": # ただアクセスしただけ
        # instance を指定することで、指定したインスタンスのデータにアクセスできます
        form = EmailChangeForm(instance=request.user)
    elif request.method == "POST":
        form = EmailChangeForm(request.POST, instance=request.user)
        if form.is_valid():  # 他の人と被ってないか
            form.save()
            # 保存後、完了ページに遷移します
...
# ...
# 以下を追加
class PasswordChangeView(auth_views.PasswordChangeView): #クラスの継承
...
# ...
# 以下を追加
class LogoutView(auth_views.LogoutView): #クラスの継承
    pass